...
if (Battle_GetState() == 3 && moveable)
{
    ...
    var b = position_meeting((x + xx), (y + yy), battle_plate)
    var input = -1
    if (dir == 270)
        input = 1
    else if (dir == 90)
    ...
    if ((a || b) && move >= 0)
    {
        ...
        if (dir == 270)
            ...
            fx = (sprite_height / 2)
        while (position_meeting((x + fx), (y + fy), block) || position_meeting((x + fx), (y + fy), battle_plate))
        {
            var mx = 0
            var my = 0
            if (dir == 270)
        ...
        if (position_meeting((x + xx), (y + yy), block) || position_meeting((x + xx), (y + yy), battle_plate))
        {
            _jump = 1
            move = 0
            if (impact){
            ...
            if position_meeting((x + xx), (y + yy), battle_plate)
            {
                var cx = 0
                var cy = 0
                if (dir == 270)
                    ...
                    cx = ((sprite_height / 2) + 1)
                var inst = instance_position((x + cx), (y + cy), battle_plate)
    ...
    repeat (abs(move) * 10)
    {
        ...
        {
            var _temp_local_var_9 = (move > 0 && position_meeting((x + xx), (y + yy), battle_plate))
